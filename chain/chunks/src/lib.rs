...
impl ShardsManager {
    ...
    pub fn process_partial_encoded_chunk_request(
        ...
        let response = if let Some(entry) = self.encoded_chunks.get(&request.chunk_hash) {
            debug!(target: "chunks", "Found {:?} in cache", request.chunk_hash.0);
            ...
            debug!(target: "chunks", "Found {:?} in ColPartialChunks", request.chunk_hash.0);
            ...
            debug!(target: "chunks", "Found {:?} in ColChunks", request.chunk_hash.0);
            // If we are archival node we might have garbage collected the
            // partial chunk yet we still keep the chunk itself.  We can get the
            // chunk and rcalculate the parts from it.
